...
@extends('layouts.app')
@section('content')
<div class="min-h-screen flex items-center justify-center bg-night">
    <div class="max-w-md w-full space-y-8 p-6 bg-white rounded-lg shadow-lg">
        <div class="text-center">
            <h2 class="text-3xl font-extrabold text-night">{{ __('Reset Password') }}</h2>
        </div>

        @if (session('status'))
            <div class="text-green-600 bg-green-100 border border-green-400 rounded p-3 text-center">
                {{ session('status') }}
            </div>
        @endif
            ...
            <div class="rounded-md shadow-sm -space-y-px">
                <div class="mb-4">
                    ...
                    <input id="email" type="email" name="email" value="{{ old('email') }}" required autocomplete="email" autofocus
                           class="mt-1 p-2 w-full border border-gray-300 rounded-md focus:ring focus:ring-indigo-200 focus:outline-none focus:ring-opacity-50 @error('email') border-red-500 @enderror">
                    @error('email')
